...
;(function($) {
  ...
  var methods = {
    ...
    _stateButton: function(index, steps) {
      ...
      if (index === 0) {
        step.find(this.opt.backButton).hide();
        step.find(this.opt.nextButton).show();
        self.find(this.opt.finishButton).hide();
      } else if (index === steps.length - 1) {
        step.find(this.opt.backButton).show();
        step.find(this.opt.nextButton).hide();
        self.find(this.opt.finishButton).show();
      } else {
        step.find(this.opt.backButton).show();
        step.find(this.opt.nextButton).show();
        self.find(this.opt.finishButton).hide();
    ...
    destroy: function() {
      return $(this).each(function() {
        ...
        if (self.data('stepy')) {
          var steps = self.data('stepy', false).find('.' + this.opt.stepClass).css('display', '');
