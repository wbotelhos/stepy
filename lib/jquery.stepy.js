  ...
  var methods = {
    ...
    _bindHeader: function() {
      ...
      if (this.opt.titleClick) {
        this.heads.on('click', methods._bindHeaderHandler.bind(this));
      } else {
        this.heads.css('cursor', 'default');
      }
    },

    _bindHeaderHandler: function() {
      var
        words   = this.heads.filter('.stepy-active').attr('id').split('-'), // TODO: try keep the number in an attribute.
        current = parseInt(words[words.length - 1], 10),
        clicked = $(this).index();

      if (clicked > current) {
        if (this.opt.next && !methods._execute.call(self, this.opt.next, clicked)) {
          return false;
        }
      } else if (clicked < current) {
        if (this.opt.back && !methods._execute.call(self, this.opt.back, clicked)) {
          return false;
        }
      }
    ...
    _createButtons: function(step, index) {
      if (index === 0) {
        if (this.steps.length > 1) {
          methods._bindNextButton.call(this, step);
        }
      } else {
        $(step).hide();

        methods._bindBackButton.call(this, step);
