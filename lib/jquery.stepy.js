  ...
  var methods = {
    init: function(settings) {
      return this.each(function() {
        ...
        this.steps = self.find('.stepy-step');
    ...
    _bindEnter: function() {
      ...
      this.steps.find('input:not(:button, :submit)').on('keypress.stepy', function(evt) {
        ...
        if (key === 13) {
          ...
          var
            step    = $(this).closest('.stepy-step'),
            hasNext = step.next('.stepy-step').length;
    ...
    destroy: function() {
      return $(this).each(function() {
        ...
        if (self.data('stepy')) {
          var steps = self.data('stepy', false).find('.stepy-step').css('display', '');
    ...
    validate: function(index) {
      ...
      var
        that     = this,
        step     = self.find('.stepy-step').eq(index),
        isValid  = true,
