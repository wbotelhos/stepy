  ...
  var methods = {
    init: function(settings) {
      return this.each(function() {
        ...
        if (id === undefined || id === '') {
          var id = methods._hash.call(that);
        ...
        if (that.opt.validate) {
          jQuery.validator.setDefaults({ ignore: that.opt.ignore });

          self.append('<div class="stepy-errors" />');
        }

        that.header = methods._header.call(that);
        that.steps  = self.children('fieldset');

        that.steps.each(function(index) {
          methods._createHead.call(that, this, index);
          methods._createButtons.call(that, this, index);
        });

        that.heads = that.header.children('li');

        that.heads.first().addClass('stepy-active');

        methods._bindFinishButton.call(that);

        if (that.opt.titleClick) {
          that.heads.click(function() {
            var
    ...
    _bindEnter: function() {
      ...
      that.steps.delegate('input[type="text"], input[type="password"]', 'keypress', function(evt) {
    ...
    _bindFinishButton: function() {
      ...
      this.finishButton = that.find('.stepy-finish');
    ...
    _createBackButton: function(nav, index) {
      var
        that       = this,
        self       = $(this),
        attributes = { href: 'javascript:void(0);', 'class': 'stepy-back', html: that.opt.backLabel };

      $('<a />', attributes).on('click.stepy', function(e) {
        e.preventDefault();

        if (!that.opt.back || methods._execute.call(that, that.opt.back, index - 1, that.steps.length)) {
    ...
    _createNextButton: function(nav, index) {
      var
        that       = this,
        self       = $(this),
        attributes = { href: 'javascript:void(0);', 'class': 'stepy-next', html: that.opt.nextLabel };

      $('<a />', attributes).on('click.stepy', function(e) {
        e.preventDefault();

        if (!that.opt.next || methods._execute.call(self, that.opt.next, index + 1, that.steps.length)) {
          methods.step.call(that, (index + 1) + 1);
        }
      }).appendTo(nav);
    },
...
debugger;
      ...
      self[0].heads.removeClass('stepy-active').eq(max).addClass('stepy-active');
    ...
    validate: function(index) {
      ...
      var
        ...
        isValid   = true,
        $title    = $('#' + self.attr('id') + '-header').children().eq(index),
