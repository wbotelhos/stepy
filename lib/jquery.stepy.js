...
;(function($) {
  ...
  var methods = {
    init: function(settings) {
      ...
      $('<a />', attributes).on('click.stepy', function(e) {
        ...
        if (!self.opt.back || methods._execute.call(self, self.opt.back, index - 1)) {
          // Suppress validation of current step when going back.
          methods.step.call(self, (index - 1) + 1, false);
        }
      }).appendTo(nav);
    ...
    /**
     * When jumping to arbitrary step, use validateStep = true;
     * otherwise jumping will be interrupted by validation failure.
     */
    step: function(index,validateStep) {
      validateStep = validateStep === undefined || validateStep != false;
      var self = this
          that = $(this),
          opt  = that[0].opt;
      ...
      // Remove Validator...
      if (opt.validate && validateStep) {
        // validate only last step
        // there is no need to validate all steps, otherwise jumping to arbitrary steps will not work.
        var stepIdxExpr = /-head-(\d+)$/ig;
        var matches = stepIdxExpr.exec(that[0].heads.filter('.stepy-active').attr('id'));
        if (matches.length == 2) {
          var lastIdx = parseInt(matches.pop());
          console.log('validating lastIdx:'+lastIdx);
          if ( lastIdx !== index && !methods.validate.call(this,lastIdx) ) {
            return;
	  }
        }
        /*
